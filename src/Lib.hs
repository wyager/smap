...
import           Data.HashMap.Strict as Map (insert, empty)
...
{-# INLINE cat #-}
cat :: forall m k v . (Hashable k, Eq k, Monad m) => NonEmpty (P.Stream (P.Of (Pair k v)) m ()) -> P.Stream (P.Of (Pair k v)) m ()
cat streams = foldM filter Map.empty streams *> return ()
    where
    filter :: HashMap k () -> P.Stream (P.Of (Pair k v)) m () -> P.Stream (P.Of (Pair k v)) m (HashMap k ())
    filter seen = fmap P.fst' . P.foldM filter' (return seen) return . S.hoist lift
...
inject :: Monad m => (k1 -> k2) -> (v1 -> v2) -> P.Stream (P.Of (Pair k1 v1)) m () ->  P.Stream (P.Of (Pair k2 v2)) m () 
inject f g = P.map (\(k :!: v) -> (f k :!: g v))

approximate :: Monad m => SipKey -> P.Stream (P.Of (Pair ByteString v)) m () ->  P.Stream (P.Of (Pair Word64 v)) m () 
...
kin :: (MonadIO m, Resource.MonadResource m) => Keyed -> P.Stream (P.Of (Pair ByteString ByteString)) m ()
kin (UnKeyed hdl) = duplicate $ force $ BS8.lines $ hin hdl
kin (Keyed hks hvs) = S.zipsWith' (\q (k P.:> ks) (v P.:> vs) -> (k :!: v) P.:> (q ks vs)) (f hks) (f hvs)
    where
    f = force . BS8.lines . hin 



keyed :: A.Parser Keyed
keyed = (keyed <|> unkeyed) <* (A.endOfInput A.<?> "more filepath characters than expected") A.<?> "Could not parse filepath"
    where
    file = stdin <|> path
    stdin = Std <$ A.char '-' 
...
catI = O.command "cat" $ O.info (value O.<**> O.helper) O.fullDesc 
    ...
    out = O.option readKeyed (O.showDefaultWith (\case (UnKeyed Std) -> "stdout"; _ -> error "Program error: default changed") <> O.metavar "OUTFILE" <> O.short 'o' <> O.long "out" <> O.value (UnKeyed Std))
